...
// AD Opt
volatile uint32_t huffDecode_ctr = 0;
...
//------------------------------------------------------------------------------
#if LESS_INLINING
static uint16 getBits(uint8 numBits, uint8 FFCheck) __attribute__((noinline));
#endif
...
//------------------------------------------------------------------------------
#if LESS_INLINING
static uint16 getBits2(uint8 numBits) __attribute__((noinline));
#endif
...
//------------------------------------------------------------------------------
#if LESS_INLINING
static int16 huffExtend(uint16 x, uint8 s) __attribute__((noinline));
#endif
...
// AD Opt: restrict
static void createWinogradQuant(int16* restrict pQuant)
{
   uint8 i;
...
#if LESS_INLINING
static void idctRows(void) __attribute__((noinline));
#endif
static void idctRows(void)
{
   ...
   for (i = 0; i < 8; i++)
   {
		 if ((pSrc[1] | pSrc[2] | pSrc[3] | pSrc[4] | pSrc[5] | pSrc[6] | pSrc[7]) == 0) // AD: suspicious
      {
...
#if LESS_INLINING
static void idctCols(void) __attribute__((noinline));
#endif
static void idctCols(void)
{
   ...
   for (i = 0; i < 8; i++)
   {
//		 if ((pSrc[1*8] | pSrc[2*8] | pSrc[3*8] | pSrc[4*8] | pSrc[5*8] | pSrc[6*8] | pSrc[7*8]) == 0) // AD: suspicious
// AD Opt
		 if ((pSrc[1*8] == 0) && (pSrc[2*8] == 0) && (pSrc[3*8] == 0) && (pSrc[4*8] == 0) && 
			 (pSrc[5*8] == 0) && (pSrc[6*8] == 0) && (pSrc[7*8] == 0) ) // AD Opt
// End AD Opt
		 {
         // Short circuit the 1D IDCT if only the DC component is non-zero
...
#if LESS_INLINING
static void upsampleCb(uint8 srcOfs, uint8 dstOfs) __attribute__((noinline));
#endif
...
#if LESS_INLINING
static void upsampleCr(uint8 srcOfs, uint8 dstOfs) __attribute__((noinline));
#endif
...
/*----------------------------------------------------------------------------*/
// Convert Y to RGB
static void copyY(uint8 dstOfs) // AD Opt: Move data 32 bits at a time. Need prolog and epilog.
{
   uint8 i;
...
#if LESS_INLINING
static void convertCb(uint8 dstOfs) __attribute__((noinline));
#endif
...
#if LESS_INLINING
static void convertCr(uint8 dstOfs) __attribute__((noinline));
#endif
...
/*----------------------------------------------------------------------------*/
#if LESS_INLINING
static void transformBlock(uint8 mcuBlock) __attribute__((noinline));
#endif
...
//------------------------------------------------------------------------------
#if LESS_INLINING
static void transformBlockReduce(uint8 mcuBlock) __attribute__((noinline));
#endif
...
//------------------------------------------------------------------------------
#if LESS_INLINING
static uint8 decodeNextMCU(void) __attribute__((noinline));
#endif
